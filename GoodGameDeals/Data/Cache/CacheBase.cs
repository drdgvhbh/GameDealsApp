...
namespace GoodGameDeals.Data.Cache {
    ...
    public abstract class CacheBase<T> {
        private readonly SemaphoreSlim _cacheFolderSemaphore =
            new SemaphoreSlim(1);
        ...
        public int MaxMemoryCacheCount {
            get => this._inMemoryFileStorage.MaxItemCount;

            set => this._inMemoryFileStorage.MaxItemCount = value;
        }
        ...
        public Task ClearAsync(TimeSpan duration) =>
            this.RemoveExpiredAsync(duration);
        ...
        public T GetFromMemoryCache(Uri uri) {
            ...
            if (this._inMemoryFileStorage.MaxItemCount > 0) {
                var msi = this._inMemoryFileStorage.GetItem(
                    fileName,
                    this.CacheDuration);
                if (msi != null) {
                    instance = msi.Item;
        ...
        /// <param name="httpMessageHandler">instance of <see cref="System.Net.Http.HttpMessageHandler" /></param>
        /// <returns>awaitable task</returns>
        public virtual async Task InitializeAsync(
            ...
            this._cacheFolder =
                await this.GetCacheFolderAsync().ConfigureAwait(false);
        }
        ...
        public async Task RemoveAsync(IEnumerable<Uri> uriForCachedItems) {
            ...
            var hashDictionary = new Dictionary<string, StorageFile>();
        ...
        public async Task RemoveExpiredAsync(TimeSpan? duration = null) {
            var expiryDuration =
                duration.HasValue ? duration.Value : this.CacheDuration;
            ...
            foreach (var file in files) {
                ...
                if (await this.IsFileOutOfDateAsync(file, expiryDuration, false)
                        .ConfigureAwait(false)) {
                    filesToDelete.Add(file);
                }
            }
        ...
        /// <param name="initializerKeyValues">key value pairs used when initializing instance of generic type</param>
        /// <returns>awaitable task</returns>
        protected abstract Task<T> InitializeTypeAsync(
            Stream stream,
            List<KeyValuePair<string, object>> initializerKeyValues = null);
        ...
        protected virtual async Task<bool> IsFileOutOfDateAsync(
            ...
            return properties.Size == 0
                   || DateTime.Now.Subtract(properties.DateModified.DateTime)
                   > duration;
        }
        ...
        private static string GetCacheFileName(Uri uri) =>
            CreateHash64(uri.ToString()).ToString();
        ...
        private async Task<T> DownloadFileAsync(
            ...
            using (var ms = new MemoryStream()) {
                ...
                await FileIO.WriteBytesAsync(baseFile, buffer.ToArray());
...
/*                // if its pre-cache we aren't looking to load items in memory
        ...
        private async Task ForceInitialiseAsync() {
            ...
            try {
                this._cacheFolder = await this._baseFolder.CreateFolderAsync(
        ...
        private async Task<T> GetFromCacheOrDownloadAsync(
            ...
            baseFile =
                await folder.TryGetItemAsync(fileName).AsTask()
                    .ConfigureAwait(this.MaintainContext) as StorageFile;

            await this.IsFileOutOfDateAsync(baseFile, this.CacheDuration)
                .ConfigureAwait(this.MaintainContext);
            if (baseFile == null || await this
                    .IsFileOutOfDateAsync(baseFile, this.CacheDuration)
                    .ConfigureAwait(this.MaintainContext)) {
                baseFile = await folder
                               .CreateFileAsync(
                                   fileName,
                                   CreationCollisionOption.ReplaceExisting)
                               .AsTask().ConfigureAwait(this.MaintainContext);

                uint retries = 0;
                try {
                    while (retries < this.RetryCount) {
                        try {
                            instance = await this.DownloadFileAsync(
                                               uri,
                                               baseFile,
                                               preCacheOnly,
                                               cancellationToken,
            ...
            if (EqualityComparer<T>.Default.Equals(instance, default(T))
                ...
                if (this._inMemoryFileStorage.MaxItemCount > 0) {
                    ...
                    var msi = new InMemoryStorageItem<T>(
                        fileName,
                        properties.DateModified.DateTime,
                        instance);
